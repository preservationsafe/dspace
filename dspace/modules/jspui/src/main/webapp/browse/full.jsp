...
<%
    ...
    String startsDate = request.getParameter("starts_date");
	String endsDate = request.getParameter("ends_date");
...
&lt;%&ndash; OK, so here we start to develop the various components we will use in the UI &ndash;%&gt;

<%@page import="java.util.Set"%>
<%@ page import="static java.lang.Integer.parseInt" %>
<dspace:layout titlekey="browse.page-title" navbar="<%=layoutNavbar %>">

	&lt;%&ndash; Build the header (careful use of spacing) &ndash;%&gt;
	<h2>
		<fmt:message key="browse.full.header"><fmt:param value="<%= scope %>"/></fmt:message> <fmt:message key="<%= typeKey %>"/> <%= value %>
	</h2>

	&lt;%&ndash; Include the main navigation for all the browse pages &ndash;%&gt;
	&lt;%&ndash; This first part is where we render the standard bits required by both possibly navigations &ndash;%&gt;


	&lt;%&ndash; Include a component for modifying sort by, order, results per page, and et-al limit &ndash;%&gt;
...
%>
        ...
        &lt;%&ndash; If we are browsing by a date, or sorting by a date, render the date selection header &ndash;%&gt;
<%
...
<div class="panel panel-primary">
	&lt;%&ndash; give us the top report on what we are looking at &ndash;%&gt;
	...
	&lt;%&ndash;  do the top previous and next page links &ndash;%&gt;
<% 
	if (bi.hasPrevPage())
	{
%>
...
%>
    ...
    &lt;%&ndash; output the results using the browselist tag &ndash;%&gt;
    <%
    	if (bix.isMetadataIndex())
    	{
    %>
	...
	%>
	&lt;%&ndash; give us the bottom report on what we are looking at &ndash;%&gt;
	...
	&lt;%&ndash;  do the bottom previous and next page links &ndash;%&gt;
<% 
	if (bi.hasPrevPage())
	{
%>
...
%>
	...
	&lt;%&ndash; dump the results for debug (uncomment to enable) &ndash;%&gt;
	&lt;%&ndash;
	<!-- <%= bi.toString() %> -->
	&ndash;%&gt;
